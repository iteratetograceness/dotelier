 ...
 * @returns {Uint8Array}
 */
export function process_image(input_bytes, k_colors) {
    const ptr0 = passArray8ToWasm0(input_bytes, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.process_image(ptr0, len0, isLikeNone(k_colors) ? 0x100000001 : (k_colors) >>> 0);
    if (ret[3]) {
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('pixel_snapper.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
